...
/**
 * Oracleのデータ取得用Lambdaを作成
 * @param stack construct
 * @param deploymentStage string
 * @param lyerArn string
 */

const createGetDbLambda = (stack: Stack, deploymentStage: string, lyerArn: string ):void => {

    const DEPLOYMENT_STAGE = deploymentStage || 'stg';
    const id = `${DEPLOYMENT_STAGE}-auth-GetDbLambda`;
    ...
    new StringParameter(stack, `${id}-LambdaArn`, {
        parameterName: `/lambda/${DEPLOYMENT_STAGE}/GetDbLambdaArn`,
        stringValue:  appLambda.functionArn,
    });
};

export default createGetDbLambda;
