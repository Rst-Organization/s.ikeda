...
/**
 * Oracleへのデータ登録用のLambdaを作成
 * @param stack construct
 * @param deploymentStage string
 * @param lyerArn string
 */

const createPutDbLambda = (stack: Stack, deploymentStage: string, lyerArn: string ):void => {

    const DEPLOYMENT_STAGE = deploymentStage || 'stg';
    const id = `${DEPLOYMENT_STAGE}-auth-PutDbLambda`;
    ...
    new StringParameter(stack, `${id}-LambdaArn`, {
        parameterName: `/lambda/${DEPLOYMENT_STAGE}/PutDbLambdaArn`,
        stringValue: appLambda.functionArn,
    });

};

export default createPutDbLambda;
